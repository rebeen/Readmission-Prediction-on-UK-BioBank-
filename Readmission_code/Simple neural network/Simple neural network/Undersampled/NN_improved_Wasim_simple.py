...
out_layers=[1,2,3]   ; out_neurons=[450,800,1600] # output layer neurons approximates decided on the possible concatenate dimensions of the previous combinations and taken as average been final output (1) and previous Concatenate dim size
...
for i in tqdm(out_layers): # testing all combinations of chosen hyperparameter search space
    ...
    for j in tqdm(out_neurons):
        ...
        lr = 0.0001 # set Chris original learning rate to Adam optimizer default 
        ...
        history=model_.fit([demo_train,diag_train,scripts_train,LTC_train],label_train
                            ,epochs=30,validation_data=([demo_test,diag_test,scripts_test,LTC_test], label_test),batch_size=64,callbacks=[early_stopping],verbose=1)
