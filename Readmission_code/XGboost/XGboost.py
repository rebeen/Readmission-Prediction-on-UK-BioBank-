...
Read_table=pd.read_csv("/home/rebeen/rebeen_data/14_08_23_Readdmision/final_readmission_dataset/Corrected_readmission_data_rerun_models/including_10_100_patients/readmission_table_include_10_100_21022024.tsv", header=0, sep='\t')
Read_table['eid'] =Read_table ['eid'].astype('int')
Read_table.set_index('eid', inplace=True)

demographics=pd.read_csv("/home/rebeen/rebeen_data/14_08_23_Readdmision/final_readmission_dataset/Corrected_readmission_data_rerun_models/including_10_100_patients/demographics.tsv", header=0, sep='\t')
demographics['eid'] =demographics['patient_id'].astype('int')
demographics.set_index('eid', inplace=True)

diagnosis =pd.read_csv("/home/rebeen/rebeen_data/14_08_23_Readdmision/final_readmission_dataset/Corrected_readmission_data_rerun_models/including_10_100_patients/Top_codes_ICD10_binary_22022024.tsv", header=0, sep='\t')
diagnosis['eid'] =diagnosis ['eid'].astype('int')
diagnosis.set_index('eid', inplace=True)

prescription =pd.read_csv("/home/rebeen/rebeen_data/14_08_23_Readdmision/final_readmission_dataset/Corrected_readmission_data_rerun_models/including_10_100_patients/BNF_binary_22022024.tsv", header=0, sep='\t')
prescription['eid'] =prescription ['eid'].astype('int')
prescription.set_index('eid', inplace=True)

Lon_term_condition =pd.read_csv("/home/rebeen/rebeen_data/14_08_23_Readdmision/final_readmission_dataset/Corrected_readmission_data_rerun_models/including_10_100_patients/LTC_binary_22022024.tsv", header=0, sep='\t')
...
readmission_train,readmission_test=train_test_split(
    readmission_dataset,
    test_size=0.2,
    stratify=readmission_dataset['readmission_status'],
    random_state=200
)

readmission_train=readmission_train.sample(frac=1,random_state=200).reset_index(drop=True)
...
X_train = np.concatenate([demo_train, diag_train, scripts_train, LTC_train, Age_at_event_train], axis=1)
X_test = np.concatenate([demo_test, diag_test, scripts_test, LTC_test, Age_at_event_test], axis=1)
...
print(f"AUROC: {auc_roc}, AUPRC: {auc_prc}, Precision: {precision}, Recall: {recall}, F1 Score: {f1}")